...
namespace Chat.BusinessLogic.DbContext
{
	public static class DbInitializer
	{
		public static async Task Seed(IServiceScope serviceScope)
		{
			...
			if (!totalUsersKeyExist)
			{
				...
				// an additional lookup is handled to resolve the name.
				// Rooms with private messages don't have a name.
				await redisDatabase.StringSetAsync("room:0:name", "General");

				await CreateDemoData(redisDatabase);
			}
		}

		private static async Task CreateDemoData(IDatabase redisDatabase)
		{
			var rnd = new Random();
			...
			var greetings = new List<string>() { "Hi", "Hey", "Hello", "Greetings human" };
